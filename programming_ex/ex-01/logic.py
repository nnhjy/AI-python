...
class AND(BinaryFormula):
  ...
  def truthValue(self,v):
    return self.subformula1.truthValue(self.subformula1.vars().intersection(v)) and self.subformula2.truthValue(self.subformula2.vars().intersection(v))
    # recursive: the valuation v should make both the subformula1 and subformula2 true
...
class OR(BinaryFormula):
  ...
  def truthValue(self,v):
    return self.subformula1.truthValue(self.subformula1.vars().intersection(v)) or self.subformula2.truthValue(self.subformula2.vars().intersection(v))
    # recursive: the valuation v should make either the subformula1 or subformula2 true
### IMPLEMENT THIS (~ 1 line)
...
class NOT:
  ...
  def truthValue(self,v):
    return not self.subformula.truthValue(self.subformula.vars().intersection(v))
    # recursive: the valuation v should make the subformula false
### IMPLEMENT THIS (~ 1 line)
...
def powerset(iterable):
    ...
    # returns a generator for all subsets of the iterable
    # "[x for x in powerset(["a","b"])] --> [(), ('a',), ('b',), ('a', 'b')]"
    s = list(iterable)
    return itertools.chain.from_iterable(itertools.combinations(s, r) for r in range(len(s)+1))

def satisfiable(f):
### IMPLEMENT THIS (10 to 15 lines total)
  set_of_all_variables = f.vars()
  for subset in powerset(set_of_all_variables):
    # a subset is a valuation v
    if f.truthValue(subset):
      return subset
      # return a valuation that makes the formula true (satisfiable)
    else:
      continue
  return False
  # return False if no feasible valuation exists (NOT satisfiable)
...
def logicalConsequence(f1,f2):
  ...
  # Proposition 4: f1 |= f2 iff. |= f1 -> f2
  # Proposition 6: |= a iff. -a is not satisfiable
  # alternative approach: all([IMPL(f1,f2).truthValue(v) for v in powerset(IMPL(f1,f2).vars())])
### IMPLEMENT THIS (~ 1 line)
